...
export function LoginForm({ className, ...props }: React.ComponentPropsWithoutRef<"form">) {
  ...
  // Обновляем defaultValues на email
  const form = useForm<SignInReq>({
    defaultValues: {
      email: DB_USER[0].username, // Используем email, предполагая, что username в моках - это email
    ...
    <div className={cn("flex flex-col gap-6", className)}>
      <Form {...form} {...props}>
        <form onSubmit={form.handleSubmit(handleFinish)} className="space-y-4">
          ...
          {/* ОБНОВЛЕНО: Используем 'email' для входа */}
          <FormField
            control={form.control}
            name="email" // Изменено с 'username' на 'email'
            rules={{ required: t("sys.login.accountPlaceholder") }}
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("sys.login.userName")}</FormLabel>
                <FormControl>
                  {/* Здесь по-прежнему отображаем username, но используем поле email */}
